...
System::Void ModelingWorkMachineWithBreakdowns::ModelingWorkForm::Btn_execution_Click(System::Object^ sender, System::EventArgs^ e)
{
	...
	double timeBeforeBreakdownMachine = getTimeBeforeBreakdownMachine(timeBreakDown_M_X, timeBreakdown_StandartDeviation); // время до поломки
	...
	while (countDetails > 0)
	{
		...
		{
			//totalTimeWorkMachine += timeBeforeNextTask; //добавляем время ожидания задания
		...
		if (timeDebugExection_oneTask < timeBeforeBreakdownMachine) 
		{
			...
			totalTimeWorkMachine += timeDebugExection_oneTask; //к общему времени работы станка добавляем время выполнения одного задания
			timeBeforeBreakdownMachine -= timeDebugExection_oneTask; //от времени до поломки отнимаем выполнение одного задания
			timeBeforeNextTask -= timeDebugExection_oneTask; //от времени до след. задания отнимаем время вып. одного задания
			countDetails--;
		}
		else
		{
			...
			//totalTimeWorkMachine += timeBeforeBreakdownMachine; //общее время работы станка + время простоя
	...
	lbl_CountBreakdown->Text = "Количество поломок: " + countBrokenDetails.ToString();
	lbl_TimeWork->Text = "Время работы станка: " + (static_cast<int>(totalTimeWorkMachine)).ToString() +" ч. " + (static_cast<int>((int)totalTimeWorkMachine % 1 * 60)).ToString() +" мин.";
	...
	lbl_totaltimeWait->Text = "Общее время простоя: " + (static_cast<int>(totalTimeWait)).ToString() + " ч. " + (static_cast<int>((int)totalTimeWait % 1 * 60)).ToString() + " мин.";
